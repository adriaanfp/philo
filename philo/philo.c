...
int	parse_args(int ac, char **av, t_rules *rules)
{
	...
	if (!safe_atol(av[1], &rules->nb_philo) || !safe_atol(av[2],
			&rules->time_to_die) || !safe_atol(av[3], &rules->time_to_eat)
		|| !safe_atol(av[4], &rules->time_to_sleep) || (ac == 6
			&& !safe_atol(av[5], &rules->must_eat)))
	{
